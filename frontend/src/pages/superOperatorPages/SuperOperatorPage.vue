<template>
  <q-page>
    ...
    <q-form @submit.prevent="updateProfile" :style="{ marginBottom: '20px' }" method="POST">
      ...
      <q-input :style="{ marginBottom: '10px' }" filled label="phone" v-model="userInfo.phone" :readonly="!isEditing"/>
...
export default {
  ...
  created() {
    axios
      .post("http://127.0.0.1:5000/api/getSuperOperatorUserData", this.userInfo)
      .then((response) => {
  ...
  methods: {
    ...
    updateProfile() {
      ...
      axios
        .post("http://127.0.0.1:5000/api/updateSuperOperatorUserData", this.userInfo)
        .then((response) => {
